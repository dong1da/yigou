  ...
  export default {
    ...
    methods: {
      ...
      handler:throttle(async function(type, disNum, cart) {
        // type：区分这三个元素
        // disNum形参可以接受3个不同的参数， + 变化量（1）  - 变化量（-1）  input最终的个数（并不是变化量）
        // cart 哪一个产品：身上有id
        // console.log('通知服务器修改购物车商品个数', type, disNum, cart)
        switch(type) {
          case 'add':
            // 带给服务器变化的量
            disNum = 1
            break;
          case 'minus':
            // 产品数大于1，才可以传递给服务器-1,否则传递个属为0
            disNum = cart.skuNum > 1 ? -1 : 0
            break;
          case 'change':
            // 用户在input中输入的是非法的(带有汉字，字母等)
      ...
      async updateChecked(cart, event) {
        try {
          ...
          await this.$store.dispatch('updateCheckById', {skuId: cart.skuId, isChecked: checked})
          this.getData()
        } catch (error) {
          alert(error.message)
        }
